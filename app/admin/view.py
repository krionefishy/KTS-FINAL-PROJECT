    ...
    async def post(self):
        ...
        try:
            ...
            if not bcrypt.checkpw(password.encode(), admin_from_db.password.encode()):
                return error_json_response(http_status=403, status="forbidden", message="Invalid credentials")

            session = await new_session(request=self.request)
            session["admin_id"] = admin_from_db.id
            session.max_age = self.request.app.config.session.lifetime

            return json_response(data={"id": admin_from_db.id, "email": email})

        except HTTPNotFound:
            return error_json_response(http_status=404, status="not_found", message="Admin not found", data={})
