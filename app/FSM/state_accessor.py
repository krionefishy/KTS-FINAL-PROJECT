...
class FsmAccessor(BaseAccessor):
    async def get_next_player(self, chat_id: int) -> int:
        try:
            async with self.app.database.session() as session:
                ...
                stmt = update(ChatSession).where(ChatSession.chat_id == chat_id).values(players=players_dct)
        ...
        try:
            async with self.app.database.session() as session:
                result = await session.execute(
                    select(ChatSession.players).where(ChatSession.chat_id == chat_id).with_for_update()
                )
                ...
                stmt = update(ChatSession).where(ChatSession.chat_id == chat_id).values(players=players_data)
    ...
    async def get_game_status(self, chat_id: int) -> bool:
        async with self.app.database.session() as session:
            result = await session.execute(select(ChatSession.is_active).where(ChatSession.chat_id == chat_id))
    ...
    async def set_game_status(self, chat_id: int):
        try:
            async with self.app.database.session() as session:
                result = await session.execute(select(ChatSession).where(ChatSession.chat_id == chat_id))

                exists = result.scalar_one_or_none()

                if exists:
                    stmt = update(ChatSession).where(ChatSession.chat_id == chat_id).values(is_active=True)
        ...
        try:
            async with self.app.database.session() as session:
                ...
                if not current_session:
                    ...
                    stmt = update(ChatSession).where(ChatSession.chat_id == chat_id).values(admin_id=admin_id)
    ...
    async def set_false_status(self, chat_id: int):
        try:
            async with self.app.database.session() as session:
                stmt = update(ChatSession).where(ChatSession.chat_id == chat_id).values(is_active=False)
    ...
    async def is_admin(self, chat_id: int, user_id: int) -> bool:
        async with self.app.database.session() as session:
            result = await session.execute(select(ChatSession.admin_id).where(ChatSession.chat_id == chat_id))
    ...
    async def set_current_theme(self, chat_id: int, theme_id: int):
        try:
            async with self.app.database.session() as session:
                stmt = update(ChatSession).where(ChatSession.chat_id == chat_id).values(current_theme=theme_id)
    ...
    async def change_game_state(self, chat_id: int, data: dict):
        try:
            async with self.app.database.session() as session:
                stmt = update(ChatSession).where(ChatSession.chat_id == chat_id).values(current_game_state=data)
    ...
    async def check_user_id(self, chat_id: int, user_id: int) -> bool:
        async with self.app.database.session() as session:
            result = await session.execute(select(ChatSession.current_game_state).where(ChatSession.chat_id == chat_id))
    ...
    async def get_themes_of_session(self, chat_id) -> list[Theme]:
        async with self.app.database.session() as session:
            result = await session.execute(select(ChatSession.game_themes).where(ChatSession.chat_id == chat_id))
            ...
            if themes:
                result_list = []
                for theme in themes:
                    result_list.append(Theme(id=theme[1], theme_name=theme[0]))
                return result_list
    ...
    async def set_themes_for_session(self, chat_id: int, themes: list[Theme]):
        try:
            async with self.app.database.session() as session:
                ...
                stmt = update(ChatSession).where(ChatSession.chat_id == chat_id).values(game_themes=new_list)
    ...
    async def check_count_answered_questions(self, chat_id: int) -> bool:
        async with self.app.database.session() as session:
            result = await session.execute(
                select(ChatSession.game_themes, ChatSession.used_theme_questions).where(ChatSession.chat_id == chat_id)
            )
    ...
    async def check_if_joined(self, chat_id: int, user_id: int):
        async with self.app.database.session() as session:
            ...
            return any(str(user_id) in player for player in players_data["players"])
    ...
    async def get_username(self, chat_id: int, user_id: int):
        return await self.app.bot.get_chat_member(chat_id, user_id)
