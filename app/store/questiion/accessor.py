...
class QuizAccessor(BaseAccessor):
    async def get_question(self, question_theme_id: int, question_price: int) -> Quesion:
    ...
    async def create_question(self, theme_name: str, question_text: str, questtion_price: int) -> Quesion:
        async with self.app.database.session() as session:
            try:
                theme_result = await session.execute(
                    select(ThemeModel._id).where(ThemeModel.theme_name == theme_name)
