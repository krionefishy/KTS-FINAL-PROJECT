...
class QuizAccessor(BaseAccessor):
    async def get_question(self, question_theme_id: int, question_price: int) -> Question:
        async with self.app.database.session() as session:
            result = await session.execute(
                select(QuestionModel).where(
                    QuestionModel.theme_id == question_theme_id, QuestionModel.price == question_price
                )
            )
    ...
    async def create_question(self, theme_name: str, question_text: str, question_price: int) -> Question:
        async with self.app.database.session() as session:
            try:
                theme_result = await session.execute(select(ThemeModel._id).where(ThemeModel.theme_name == theme_name))

                theme_id = theme_result.scalar_one_or_none()
                if theme_id is None:
                    raise HTTPBadRequest(text="Theme does not exists", content_type="application/json")
                ...
                if result is not None:
                    raise HTTPConflict(text=f"Question {question_text} already exists", content_type="application/json")
        ...
        try:
            async with self.app.database.session() as session:
                exists = await session.execute(
                    select(QuestionModel).where(
                        QuestionModel.question_id == question_id, QuestionModel.theme_id == theme_id
                    )
                )

                exists = exists.scalar_one_or_none()
                if exists is None:
                    raise HTTPNotFound(text="This question does not exists", content_type="application/json")

                stmt = delete(QuestionModel).where(
                    QuestionModel.theme_id == theme_id, QuestionModel.question_id == question_id
                )
                await session.execute(stmt)
                await session.commit()
        except Exception as e:
    ...
    async def get_random_question_for_game(self, chat_id: int, theme_id: int, price: int):
        try:
            async with self.app.database.session() as session:
                result = await session.execute(
                    select(ChatSession.used_theme_questions).where(ChatSession.chat_id == chat_id)
                )
                ...
                result = await session.execute(
                    ...
                    .where(QuestionModel.theme_id == theme_id, QuestionModel.price == price)
                ...
                if question:
                    ...
                    stmt = update(ChatSession).where(ChatSession.chat_id == chat_id).values(used_theme_questions=lst)
    ...
    async def get_question_price(self, question_id: int) -> int:
        async with self.app.database.session() as session:
            result = await session.execute(select(QuestionModel.price).where(QuestionModel.question_id == question_id))
