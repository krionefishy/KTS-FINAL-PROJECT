...
from app.store.database.modles import Question
...
class GameHandler:
    ...
    async def start_game(self, chat_id: int):
        ...
        message = await self.bot.send_message(
            chat_id=chat_id, text="Начинаем новую игру. Присоединяйстесь", reply_markup=create_join_kb()
        )
    ...
    async def stop_game(self, chat_id: int):
        ...
        return {"winner_id": winner_id, "winner_score": winner_score, "all_players": players_list}
    ...
    async def _waiting_for_players(self, chat_id: int, message_id: int):
        ...
        if count_players == 0:
            await self.bot.send_message(chat_id=chat_id, text="❌ Не удалось начать игру - нет участников.")
    ...
    async def _start_round(self, chat_id: int):
        await self.bot.send_message(chat_id=chat_id, text="Начинаем игру!")
        ...
        state_data = {"state": "selecting_theme", "current_player": user_id, "current_question": None}
        ...
        try:
            ...
            if is_already_joined:
                await self.bot.answer_callback_query(
                    callback_query_id=callback_query["id"], text="Вы уже присоединились к игре"
                )
                return

            await self.bot.answer_callback_query(
                callback_query_id=callback_query["id"], text="Вы присоединились к игре"
            )
            ...
            if await self.fsm.get_game_status(chat_id):
                ...
                await self.bot.send_message(chat_id=chat_id, text="Пока что нельзя подключиться")
        except Exception as e:
            logging.error(f"error join game {e}")
            await self.bot.send_message(chat_id=chat_id, text="Ошибка подключения")

    async def handle_theme_selection(self, chat_id: int, theme_id: int, user_id: int, callback_query: dict):
        is_correct_player = await self.fsm.check_user_id(chat_id, user_id)

        current_state = await self.fsm.get_game_state(chat_id)

        if current_state and current_state["state"] != "selecting_theme":
            return
        ...
        state_data = {"state": "selecting_question", "current_player": user_id, "current_question": None}
        await self.fsm.change_game_state(chat_id, state_data)
    ...
    async def handle_question_price(self, chat_id: int, price: int, user_id: int, callback_query: dict):
        is_correct_player = await self.fsm.check_user_id(chat_id, user_id)

        current_state = await self.fsm.get_game_state(chat_id)

        if current_state and current_state["state"] != "selecting_question":
            return
        ...
        if theme_id > 0:
            question: Question = await self.bot.app.store.quesions.get_random_question_for_game(
                chat_id, theme_id, price
            )
            if question is None:
                await self.bot.send_message(
                    chat_id=chat_id,
                ...
                return
            state_data = {"state": "selecting_answer", "current_player": user_id, "current_question": question.id}
            await self.fsm.change_game_state(chat_id, state_data)
            username = await self.fsm.get_username(chat_id, user_id)
            username = username if username else "Игрок"
    ...
    async def handle_answer(self, chat_id: int, user_id: int, question_id: int, idx: int, callback_query: dict):
        is_correct_player = await self.fsm.check_user_id(chat_id, user_id)

        current_state = await self.fsm.get_game_state(chat_id)

        if current_state and current_state["state"] != "selecting_answer":
            return
        ...
        if is_correct:
            ...
            await self.bot.send_message(
                chat_id=chat_id, text=f"✅ Правильно! +{points} очков\n Правильный ответ: {chosen_answer}"
            )
            ...
            await self.bot.send_message(chat_id=chat_id, text=message, parse_mode="HTML")
            await self.bot.answer_callback_query(
                callback_query_id=callback_query["id"], text="❌ Неверный ответ! Ход передан следующему игроку"
            )
        return
    ...
    async def _next_turn(self, chat_id: int):
        ...
        state_data = {"state": "selecting_theme", "current_player": next_player_id, "current_question": None}
        await self.fsm.change_game_state(chat_id, state_data)
    ...
    async def process_callback(self, callback_query: dict):
        ...
        match action:
            case "join_game":
                await self.handle_join_game(chat_id, user_id, callback_query)

            case "theme":
                theme_id = int(params[0])
                await self.handle_theme_selection(chat_id, theme_id, user_id, callback_query)

            case "question":
                question_price = int(params[0])
                await self.handle_question_price(chat_id, question_price, user_id, callback_query)

            case "answer":
                question_id = int(params[0])
                idx = int(params[1])
                await self.handle_answer(chat_id, user_id, question_id, idx, callback_query)
